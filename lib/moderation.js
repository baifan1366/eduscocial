...
export async function updateUserEmbedding(userId, postContent, postTitle) {
  try {
    ...
    if (existingEmbedding?.embedding) {
      // Check embedding dimensions before blending
      console.log('Existing embedding dimensions:', existingEmbedding.embedding.length);
      console.log('New post embedding dimensions:', postEmbedding.length);

      // Only blend if dimensions match, otherwise replace
      let finalEmbedding;
      if (existingEmbedding.embedding.length === postEmbedding.length) {
        // Blend new embedding with existing (70% existing, 30% new)
        finalEmbedding = blendEmbeddings(existingEmbedding.embedding, postEmbedding, 0.7, 0.3);
      } else {
        console.warn('Embedding dimensions mismatch, using new embedding instead of blending');
        finalEmbedding = postEmbedding;
      }
