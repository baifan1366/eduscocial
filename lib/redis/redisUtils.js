 ...
 * @param {number} currentDbCount - Current view count from database (optional)
 * @returns {Promise<number>} - The updated view count
 */
export async function incrementPostView(postId, currentDbCount = null) {
  ...
  try {
    ...
    if (!exists && currentDbCount !== null) {
      // Initialize Redis with the current database count
      await redis.set(key, currentDbCount);
      console.log(`[incrementPostView] Initialized Redis key ${key} with database count: ${currentDbCount}`);
    }
    ...
    // Fallback: return current count + 1
    return (currentDbCount || 0) + 1;
  }
}
 ...
 * @returns {Promise<number>} - The like count from database
 */
async function getPostLikeCountFromDB(postId) {
  try {
    if (!supabase) {
    ...
    if (error) {
      console.error('Error getting post like count from database:', error);
      return 0;
    }

    return data ? data.length : 0;
  } catch (error) {
    console.error('Error in getPostLikeCountFromDB:', error);
    return 0;
  }
}
...
export async function cachePostHotComments(postId, comments, ttlInSeconds = 1800) {
  ...
  try {
    ...
    if (!Array.isArray(comments)) {
      console.error(`[cachePostHotComments] Invalid comments data for ${postId}:`, typeof comments);
      return;
    }

    // Ensure all comments are serializable
    const serializedComments = JSON.stringify(comments);

    // Verify the serialization worked correctly
    if (serializedComments === '[object Object]' || serializedComments.includes('[object Object]')) {
      console.error(`[cachePostHotComments] Serialization failed for ${postId}, data contains [object Object]`);
      console.error(`[cachePostHotComments] Original data:`, comments);
      return;
    }

    await redis.set(key, serializedComments);
    await redis.expire(key, ttlInSeconds);
    console.log(`[cachePostHotComments] Successfully cached ${comments.length} comments for post ${postId}`);
  } catch (error) {
...
export async function getCachedPostHotComments(postId) {
  ...
  try {
    ...
    const dataStr = String(commentsData);
    if (dataStr === '[object Object]' || dataStr.startsWith('[object')) {
      console.warn(`[getCachedPostHotComments] Invalid cached data for ${postId}, clearing cache`);
    ...
    try {
      ...
      console.error(`[getCachedPostHotComments] JSON parse error for ${postId}:`, parseError);
      console.error(`[getCachedPostHotComments] Invalid data:`, commentsData.substring(0, 100));
      // Clear the invalid cache entry
      await redis.del(key);
      return null;
    }
 ...
 * @param {string} postId - The post ID
 * @returns {Promise<number|null>} - The cached view count or null if not found
 */
export async function getCachedPostViewCount(postId) {
  const key = `post:${postId}:views`;
  ...
  try {
    ...
    console.error('Error getting cached post view count:', error);
    return null;
  }
}

/**
 * Sync post view count from database to Redis cache
 * @param {string} postId - The post ID
 * @param {number} dbViewCount - The view count from database
 * @returns {Promise<boolean>} - Success status
 */
export async function syncPostViewCountToCache(postId, dbViewCount) {
  const key = `post:${postId}:views`;

  try {
    await redis.set(key, dbViewCount);
    console.log(`[syncPostViewCountToCache] Synced view count for post ${postId}: ${dbViewCount}`);
    return true;
  } catch (error) {
    console.error('Error syncing post view count to cache:', error);
    return false;
  }
}
...
export async function decrementCachedLikeCount(postId) {
  ...
  try {
    // Get current value first to check if it exists and is valid
    const currentValue = await redis.get(key);

    if (currentValue === null) {
      // If key doesn't exist, initialize from database
      const dbCount = await getPostLikeCountFromDB(postId);
      const newCount = Math.max(0, dbCount - 1);
      await redis.set(key, newCount.toString());
      await redis.expire(key, 3600);
      return newCount;
    }

    const currentCount = parseInt(currentValue, 10) || 0;
    ...
    // Double-check that we didn't go below 0 due to race conditions
    if (newCount < 0) {
      await redis.set(key, '0');
      await redis.expire(key, 3600);
      return 0;
...
export async function bufferLikeOperation(userId, postId, action) {
  ...
  try {
    // Check for existing pending operations for this user and post
    const pendingOps = await redis.lrange('pending_like_operations', 0, -1);
    let existingOpData = null;
    let existingOpIndex = -1;

    for (let i = 0; i < pendingOps.length; i++) {
      try {
        let op;
        if (typeof pendingOps[i] === 'string') {
          op = JSON.parse(pendingOps[i]);
        } else {
          op = pendingOps[i];
        }

        if (op.user_id === userId && op.post_id === postId) {
          existingOpData = op;
          existingOpIndex = i;
          break;
        }
    ...
    // If user is trying to do the same action again, ignore it
    if (existingOpData && existingOpData.action === action) {
      console.log(`Ignoring duplicate ${action} operation for user ${userId} on post ${postId}`);
      return;
    }

    // If there's an existing opposite operation, remove it (this cancels out the actions)
    if (existingOpData && existingOpData.action !== action) {
      // Remove the existing operation
      await redis.lrem('pending_like_operations', 1, pendingOps[existingOpIndex]);
      console.log(`Cancelled ${existingOpData.action} with ${action} for user ${userId} on post ${postId}`);

      // Undo the counter change from the previous operation
      if (existingOpData.action === 'like') {
        await decrementCachedLikeCount(postId);
      } else if (existingOpData.action === 'unlike') {
        await incrementCachedLikeCount(postId);
      }

      // Don't add a new operation since they cancel out
      // The net effect is that the user is back to the original state
      return;
    }

    // No existing operation, add new operation
        ...
        if (actualResult) {
          try {
            ...
            if (typeof actualResult === 'string') {
              operation = JSON.parse(actualResult);
            } else {
            ...
            console.error('Error parsing like operation JSON:', parseError, 'Raw result:', actualResult);
          }
        }
      }
    }
      ...
      for (const voteOp of voteOperations) {
        try {
          if (voteOp.action === 'delete') {
            // Delete the vote record for unlike operation
            await supabase
              .from('votes')
              .delete()
            ...
            // For like operation, check if vote already exists
            ...
            if (!existingVote) {
              // Only insert if vote doesn't exist
              await supabase
                .from('votes')
                .insert(voteOp);
            }
