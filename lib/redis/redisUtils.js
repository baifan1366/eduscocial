...
export async function getHotPosts() {
  try {
    ...
    console.error('Redis error in getHotPosts:', error);
    return null;
  }
}
...
export async function storeUserInterests(userId, interests) {
  try {
    ...
    console.error(`Redis error in storeUserInterests for user ${userId}:`, error);
...
export async function getUserInterests(userId) {
  try {
    ...
    console.error(`Redis error in getUserInterests for user ${userId}:`, error);
    return null;
  }
}
...
export async function isNewUser(userId) {
  try {
    ...
    // Check if redis.exists is available
    let hasInterests = false;
    if (typeof redis.exists === 'function') {
      hasInterests = await redis.exists(interestsKey);
    } else {
      // Fallback: try to get the interests and check if they exist
      const interests = await redis.get(interestsKey);
    ...
    console.error(`Redis error in isNewUser for user ${userId}:`, error);
    // Return null to indicate error, so the component can handle it appropriately
    return null;
  }
}
...
export async function cachePersonalizedRecommendations(userId, posts, ttlInSeconds = 900) {
  try {
    ...
    console.error(`Redis error in cachePersonalizedRecommendations for user ${userId}:`, error);
...
export async function getPersonalizedRecommendations(userId) {
  try {
    ...
    console.error(`Redis error in getPersonalizedRecommendations for user ${userId}:`, error);
    return null;
  }
} 