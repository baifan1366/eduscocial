...
/**
 * Track user actions for embedding updates
 * @param {string} userId - The ID of the user
 * @param {string} action - Action type (view_post, like_post, etc.)
 * @param {string} targetId - Target ID (post ID, etc.)
 * @param {object} metadata - Additional metadata
 * @returns {Promise<void>}
 */
export async function trackUserAction(userId, action, targetId, metadata = {}) {
  if (!userId) return;
  
  try {
    // Buffer action to Redis for batch processing
    await bufferUserAction(
      userId,
      action,
      action.includes('_') ? action.split('_')[1] + 's' : null, // e.g., view_post -> posts
      targetId,
      null, // oldData
      null, // newData
      { 
        ...metadata,
        tracked_for_embedding: true,
        timestamp: Date.now() 
      }
    );
    
    // Optionally, we could add logic to trigger immediate embedding update
    // for certain high-value actions, but that would typically be handled by
    // the batch process to avoid too many updates
  } catch (error) {
...
/**
 * Track profile update for embedding update
 * @param {string} userId - The ID of the user
 * @param {object} oldProfile - Previous profile data
 * @param {object} newProfile - New profile data
 * @returns {Promise<void>}
 */
export async function trackProfileUpdate(userId, oldProfile, newProfile) {
  if (!userId) return;
  ...
  try {
    ...
    await bufferUserAction(
      ...
      { 
        tracked_for_embedding: true,
        requires_embedding_update: true,
        timestamp: Date.now() 
      }
    );
    
    // Invalidate user embedding cache to force regeneration on next request
