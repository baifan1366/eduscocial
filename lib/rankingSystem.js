...
export async function rankPosts(posts, options = {}) {
  ...
  try {
    ...
    // Fetch full post data with author information
    ...
    let rankedPosts = fullPosts.map(post => {
      ...
      // Extract author and board info from the joined data
      const author = post.users;
      const board = post.boards;

      // Remove the joined data from the post object to avoid confusion
      const { users, boards, ...cleanPost } = post;
