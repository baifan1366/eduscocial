...
export const usersApi = {
  ...
  getNotifications: (userId, params = {}) => apiClient.get(`/api/users/${userId}/notification`, params),
  
  // Mark notifications as read
  markNotificationsRead: (userId, data) => apiClient.patch(`/api/users/${userId}/notification`, data),
  
  // Create notification for a user
  createNotification: (userId, data) => apiClient.post(`/api/users/${userId}/notification`, data)
};
...
export const notifyApi = {
  ...
  getById: (id) => apiClient.get(`/api/notify/${id}`),
  
  // Mark notification as read
  markAsRead: (id) => apiClient.patch(`/api/notify/${id}`),
  
  // Delete notification
  delete: (id) => apiClient.delete(`/api/notify/${id}`),
  
  // Get realtime notification config
  getRealtimeConfig: () => apiClient.get('/api/notify/realtime'),
  
  // Create SSE connection for realtime notifications
  createSSEConnection: () => {
    if (typeof window === 'undefined') return null;
    
    const eventSource = new EventSource('/api/notify/sse', { 
...
export const settingsApi = {
  settings: {
    // 上传学校信息
    uploadSchoolInfo: (data) => apiClient.post('/api/settings/school-info', data),
    
    // 获取学校信息
    getSchoolInfo: () => apiClient.get('/api/settings/school-info')
  }
}
