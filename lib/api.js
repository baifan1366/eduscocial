...
const boardsApi = {
  ...
  // Board follow operations
  followBoard: async (boardSlug) => {
  ...
  getFollowStatus: async (boardSlug) => {
    const response = await fetch(`/api/boards/${boardSlug}/follow`);
...
const settingsApi = {
  ...
  updateSetting: async (path, value) => {
    // This function allows updating a specific setting by path
    // First get current settings, then update the specific path
    const currentResponse = await settingsApi.get();
    const currentSettings = currentResponse.settings;

    // Create a deep copy and update the specific path
  ...
  updateTheme: async (theme) => {
    // Specialized function for updating only the theme column
    // No longer updates settings.preferences.theme
  ...
  uploadSchoolInfo: async (data) => {
    ...
    if (!response.ok) {
      const error = await response.json();
      throw new Error(error.error || 'Failed to upload school info');
    }
...
const creditPlansApi = {
  getAll: async (params = {}) => {
    ...
    const query = queryParams.toString() ? `?${queryParams.toString()}` : '';
    const response = await fetch(`/api/credit-plans${query}`);
    return response.json();
  },
  getByPlanId: async (planId) => {
    ...
    if (!response.ok) {
      const error = await response.json();
      throw new Error(error.message || 'Failed to fetch credit plan');
    }
...
const paymentsApi = {
  updateOrderStatus: async (orderId, data) => {
    const response = await fetch(`/api/business/credit-orders/${orderId}`, {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(data)
    });
...
const stripeApi = {
  ...
  getPaymentStatus: async (data) => {
    ...
    if (data.paymentIntentId) params.set('payment_intent', data.paymentIntentId);
  ...
  refreshPaymentStatus: async (data) => {
...
const creditOrdersApi = {
  ...
  getById: async (id) => {
    ...
    if (!response.ok) {
      const error = await response.json();
      throw new Error(error.message || 'Failed to fetch credit order');
    }
